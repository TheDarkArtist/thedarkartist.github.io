...
const EditProfile = ({ setEditProfile }) => {
  ...
  const isUsernameUnique = async () => {
    try {
      const q = query(collection(db, 'users'), where('username', '==', inputRefs.username.current.value))
      const querySnapshot = await getDocs(q)
      if(inputRefs.username.current.value == querySnapshot.docs[0].data().username)
        return true
      return querySnapshot.empty
    } catch (error) {
      console.error('error checking uniqueness of username');
      console.error(error);
    }
  ...
  const isEmailUnique = async () => {
    try {
      const q = query(collection(db, 'users'), where('email', '==', inputRefs.email.current.value))
      const querySnapshot = await getDocs(q)
 if(inputRefs.email.current.value == querySnapshot.docs[0].data().email)
        return true
      return querySnapshot.empty
    } catch (error) {
      console.error('error checking uniqueness of email');
      console.error(error);
    }

  }

  const isPhoneUnique = async () => {
    try {
      const q = query(collection(db, 'users'), where('phoneNumber', '==', inputRefs.phone.current.value))
      const querySnapshot = await getDocs(q)
 if(inputRefs.phone.current.value == querySnapshot.docs[0].data().phoneNumber)
        return true
      return querySnapshot.empty
    } catch (error) {
      console.error('error checking uniqueness of phone');
      console.error(error);
    }
  ...
  const handleSubmit = async (e) => {
    ...
    const uniqueEmail = await isEmailUnique();
    const uniqueUsername = await isUsernameUnique();
    const uniquePhone = await isPhoneUnique();
    ...
    if(!uniquePhone){
      alert('Phone Number Alreay Exists!')
      return
    }
